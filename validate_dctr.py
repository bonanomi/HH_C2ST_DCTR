...
def plot_closure(channel, var, values, y, raw_before, raw_after, new_dctr, bins,
    ...
    ax.step(centers, mc_after, where='mid', label='MC DY corrected', color="tab:red")
...
def make_bin_edges(values, n_bins, plot_range=None):
    """Return equal-width histogram edges.

    If ``plot_range`` is provided, it must be ``(xmin, xmax)`` and exactly that range is
    divided into ``n_bins`` equal-width bins. Otherwise the finite min/max of ``values`` is
    used. Values outside the chosen range are excluded from the closure histogram (and from
    the shape-normalization integral, when ``normalization='shape'``).
    """
    values = np.asarray(values)
    finite_values = values[np.isfinite(values)]
    if finite_values.size == 0:
        raise ValueError('Cannot build histogram bins: variable contains no finite values')

    if plot_range is None:
        xmin = float(np.min(finite_values))
        xmax = float(np.max(finite_values))
    else:
        xmin, xmax = map(float, plot_range)

    if not np.isfinite(xmin) or not np.isfinite(xmax) or xmax <= xmin:
        raise ValueError(f'Invalid plotting range: ({xmin}, {xmax})')
    if n_bins < 1:
        raise ValueError('--bins must be >= 1')

    return np.linspace(xmin, xmax, n_bins + 1, dtype=np.float64)
...
def main():
    ...
    ap.add_argument('--bins', type=int, default=20,
                    help='number of equal-width bins in the selected plotting range')
    ap.add_argument('--range', dest='plot_range', nargs=2, type=float, metavar=('MIN', 'MAX'),
                    default=None,
                    help='optional plotting range. Example: --range 0 200 --bins 60. '
                         'If omitted, use the finite min/max of each variable.')
    ...
    for channel in cfg.CHANNELS:
        ...
        for var in args.vars:
            ...
            bins = make_bin_edges(values, args.bins, args.plot_range)
            ...
            plt.close(fig)
            print(
                f'[{channel}, {var}] range=({bins[0]:.6g}, {bins[-1]:.6g}), bins={args.bins}, '
